...
class OrdersViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,ChartViewDelegate {
    ...
    var currentCustomer : Customer! {
        ...
        didSet{
            print(currentCustomer.customerId)
        }
    }
    
    @IBAction func selectChanged(sender: UISegmentedControl) {
        ...
        -> UITableViewCell
    {
        ...
        if orderList.count > indexPath.row
        {
            cell.lbOrderNo.text = self.orderList[indexPath.row].orderNo
            cell.lbOrderNo.sizeToFit()
            cell.lbOrderTime.text = self.orderList[indexPath.row].orderTime
            cell.lbOrderNo.sizeToFit()
            cell.lbOrderAmount.text = "Ghc: \(self.orderList[indexPath.row].totalPayAmount)"
            cell.lbOrderNo.sizeToFit()
        }
        return cell
    }
    ...
    func headRefresh(){
        ...
        Alamofire.request(Router.GetPrepayOrderList(self.currentCustomer.customerId, 1, self.pageCount)).responseArray { (orderList : [Order]?, error: ErrorType?) -> Void in
            
            
            if let list = orderList{
                self.orderList.removeAll(keepCapacity: false)
    ...
    func footRefresh(){
        ...
        Alamofire.request(Router.GetPrepayOrderList(self.currentCustomer.customerId, self.currentPageIndex, self.pageCount)).responseArray { (orderList : [Order]?, error: ErrorType?) -> Void in
    ...
    func LoadData()
    {
        ...
        Alamofire.request(Router.GetPrepayOrderAmountByMonth(self.currentCustomer.customerId)).responseArray{
            (orderList : [MonthOrder]?, error: ErrorType?) -> Void in
    ...
    func setChart(monthorderlist:[MonthOrder]){
        var dataEntries:[BarChartDataEntry] = []
        var xValues:[String] = []
        
        for i in 0..<monthorderlist.count{
            let dataEntry = BarChartDataEntry(value: monthorderlist[i].Amount, xIndex: i)
